 ...
 * #wakeup()} notifies.
 *
 * <p>Readiness is re-checked and the wait entered while holding {@link VirtualNet#LOCK}, so a write
 * that lands between the two cannot spend its notification on nobody; the bounded park is the safety
 * net CheerpJ needs (see {@link VirtualNet#PARK_MILLIS}).
 */
final class VirtualSelector extends AbstractSelector {
  ...
  protected SelectionKey register(AbstractSelectableChannel channel, int ops, Object attachment) {
    ...
    synchronized (VirtualNet.LOCK) {
      keys.add(key);
      // A key registered on a channel that is already readable has to reach a selector that is
      // parked with no timeout to fall back on.
  ...
  private int doSelect(long timeout) throws IOException {
    ...
    while (true) {
      ...
      synchronized (VirtualNet.LOCK) {
        ...
        Trace.stalled(since, "select waiting, " + keys.size() + " keys");
        if (wakeupPending) {
          wakeupPending = false;
          return 0;
        }
        if (timeout == 0) return 0;
        long park = VirtualNet.PARK_MILLIS;
        if (deadline > 0) {
