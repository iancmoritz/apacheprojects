...
public final class VirtualNet {
  ...
  /**
   * How long a blocked read, write, accept or select parks before looking again.
   *
   * <p>Every state change in this package notifies {@link #LOCK}, so this is a safety net rather than
   * a poll interval -- but CheerpJ needs it to be finite. The JVM runs on the browser's thread, and
   * threads parked with no timer pending are a state its scheduler spins in: one missed notification
   * would not slow the cluster down, it would freeze the tab for good.
   */
  static final long PARK_MILLIS = Long.getLong("hadoopwasm.park.ms", 250L);
  ...
  /** Parks the calling thread until something in this package moves, or {@link #PARK_MILLIS}. */
  static void pause() {
    synchronized (LOCK) {
      try {
