 ...
 * loop. Selecting this provider with {@code -Djava.nio.channels.spi.SelectorProvider} gives Flink's
 * blob server and netty shuffle stack channels that talk over in-process pipes instead of real
 * sockets, which is all a MiniCluster needs: both ends of every connection are in this one JVM.
 *
 * <p>This package is the one in {@code ../../../../../spark/java/.../net}, which had to solve the
 * same problem for Spark, with only its package renamed.
 */
