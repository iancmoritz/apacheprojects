...
// serve it from here to keep the dev server's URLs identical to the deployed site's.
...
export default defineConfig({
  ...
  build: {
    target: "es2022",
    sourcemap: true,
    // Every project's build is merged into one dist/ by ../scripts/assemble.mjs, which fails on a
    // collision, so this build owns exactly two paths: the /echarts/ document and /_echarts/.
    assetsDir: "_echarts/assets",
    rollupOptions: { input: { echarts: "echarts/index.html" } },
  },
});
