...
public class BufferPool
{
    ...
    /**
     * Browser (CheerpJ) builds: direct buffers do not sign-extend single-byte reads, so the
     * pool is bypassed entirely and every request is served by a plain heap buffer.
     */
    ...
    {
        if (ON_HEAP_ONLY || bufferType == BufferType.ON_HEAP)
    ...
    {
        return ON_HEAP_ONLY ? allocate(size, BufferType.ON_HEAP) : localPool.get().tryGet(size, false);
    }

    public ByteBuffer tryGetAtLeast(int size)
    {
        return ON_HEAP_ONLY ? allocate(size, BufferType.ON_HEAP) : localPool.get().tryGet(size, true);
    }

    private ByteBuffer allocate(int size, BufferType bufferType)
    {
        updateOverflowMemoryUsage(size);
        return ON_HEAP_ONLY || bufferType == BufferType.ON_HEAP
