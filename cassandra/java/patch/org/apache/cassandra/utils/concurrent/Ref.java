...
public final class Ref<T> implements RefCounted<T>
{
    ...
    static final class State extends PhantomReference<Ref>
    {
        ...
        // cassandra-wasm: CheerpJ computes the wrong field offset for fields declared in
        // subclasses of PhantomReference, so an AtomicIntegerFieldUpdater over this field never
        // wins its CAS and every release looks like a double release.  A separate AtomicInteger
        // has the same semantics and does work.
        private final java.util.concurrent.atomic.AtomicInteger released = new java.util.concurrent.atomic.AtomicInteger();
        ...
        {
            if (DEBUG_ENABLED && released.get() == 1)
                debug.log(toString());
