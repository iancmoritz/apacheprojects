...
public final class PathUtils
{
    ...
    {
        return browserSpace(handleLargeFileSystem(tryOnFileStore(path, getSpace)));
    }

    public static long tryGetSpace(Path path, IOToLongFunction<FileStore> getSpace, Consumer<IOException> orElse)
    {
        return browserSpace(handleLargeFileSystem(tryOnFileStore(path, getSpace, orElse)));
    }

    // cassandra-wasm: CheerpJ's IndexedDB-backed file system has no FileStore, so every space
    // query answers 0 and Cassandra refuses to flush.  Report a fixed quota instead.
    private static final long WASM_SPACE = Long.getLong("cassandra.wasm.fs_space_bytes", 0L);

    private static long browserSpace(long size)
    {
        return size == 0 ? WASM_SPACE : size;
    }
