...
public final class SocketFactory
{
    ...
    enum Provider
    {
        NIO
        {
            ...
            {
                // cassandra-wasm: a browser has no nio selectors (SelectorProvider.provider()
                // throws under CheerpJ), and this port never opens a socket -- internode messages
                // leave through MessagingService.outboundSink.  DefaultEventLoopGroup is a plain
                // thread-and-queue event loop with no selector, so the event loops Netty hands to
                // the messaging code still work for scheduled tasks.
                return new io.netty.channel.DefaultEventLoopGroup(threadCount, new ThreadPerTaskExecutor(threadFactory));
            }
