...
public class MessagingService extends MessagingServiceMBeanImpl
{
    ...
    /**
     * Browser (CheerpJ) builds have no TCP stack: nothing can bind a listening socket, so the
     * inbound sockets are never opened. Messages are instead handed to this instance through
     * {@link #inboundSink} by the worker's postMessage transport, and outgoing messages are
     * intercepted through {@link #outboundSink}.
     */
    private static final boolean NO_SOCKETS = Boolean.getBoolean("cassandra.wasm.no_sockets");
